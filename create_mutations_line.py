# coding=utf-8
"""mutation++ line mutation generator

Usage:
  create_mutations_line.py <filename> [options]
...
def create_mutations_line(source_filename, patch_directory, first_line, last_line):
    # read source file
    ...
    for line_number in bar(range(len(source_lines))):
        ...
        # write mutation
        if not in_comment:
            patch_filename = '{patch_directory}/{original_source}_line_{patch:05d}.patch'.format(
                patch_directory=patch_directory, original_source=os.path.basename(source_filename), patch=line_number + 1
            )
            create_patch(patch_filename, source_filename, source_lines, line_number, None)
...
if __name__ == '__main__':
    ...
    create_mutations_line(filename, directory, first_line, last_line)
